...
export class Thor {
  ...
  private findAngle(a: [number, number], b: [number, number]): number {
    ...
    let result = Math.acos(
      this.dotProduct(a, b) / this.productMagnitudes(a, b)
    );

    return result;
  }

  private findClosestCompassDirection(
    thorToLightVector: [number, number]
    ...
    this.compass.forEach((compassPoint) => {
      let angle = this.findAngle(thorToLightVector, [
        compassPoint[0],
        compassPoint[1],
      ]);
  ...
  private calculateThorsNewPosition(
    currentPosition: [number, number],
    compassPoint: [number, number, string]
  ): [number, number] {
    let newPosition: [number, number] = [
      currentPosition[0] + compassPoint[0],
      currentPosition[1] + compassPoint[1],
    ];
    return newPosition;
  }
  ...
  private gameLoop() {
    ...
    while (true) {
      ...
      const thorToLightVector: [number, number] = [
        this.lightX - this.thorCurrentPosition[0],
        this.lightY - this.thorCurrentPosition[1],
      ];
      ...
      let newPosition = this.calculateThorsNewPosition(
        this.thorCurrentPosition,
        direction
      );
